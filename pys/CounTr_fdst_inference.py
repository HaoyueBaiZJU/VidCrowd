...
def cat_imgs(img_blocks, crop_size, m, n):
    img_stacks = []
    for i in range(m):
        img_stack = np.concatenate(img_blocks[i*n:i*n+n], axis=1)
        img_stacks.append(img_stack)
    img_out = np.concatenate(img_stacks, axis=0)
    return img_out
...
checkpoint = torch.load(f'/mnt/home/zpengac/USERDIR/count/drone_benchmark/counTR_fdst_300/best-checkpoint-279epoch.bin')
    ...
    with torch.cuda.amp.autocast(): #native fp16
        for i in range(m):
            for j in range(n):
                ...
                #pred_blocks.append(preds.detach().cpu().numpy().squeeze())
                
    #preds_total = cat_imgs(pred_blocks, crop_size, m, n)
    
    #for pred, dmap in zip(preds_total, density_maps):
    #pred_array = preds_total#.detach().cpu().numpy().squeeze()
    #dmap_array = density_maps.detach().cpu().numpy().squeeze()
