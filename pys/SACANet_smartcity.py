...
class TrainGlobalConfig:
    ...
    folder = 'SACANet-9.1-smartcity-448-trial2'
    downsample = args.downsample
...
if args.mode == 'train':
    ...
    test_net = SACANet().cuda()
    test_net = nn.DataParallel(test_net)

    checkpoint = torch.load(f'/mnt/home/zpengac/USERDIR/count/drone_benchmark/SACANet-9.1-smartcity-448-trial2/best-checkpoint-014epoch.bin')
    ...
    for imgs, dmaps, fns, points in tqdm(val_loader):
        with torch.no_grad():
            ...
            preds = test_net(imgs)*args.downsample*args.downsample / LOG_PARA
        dmaps = dmaps*args.downsample*args.downsample / LOG_PARA
