...
class Head(nn.Module):
    ...
    def __init__(self, image_size, patch_size,**kwargs):
        ...
        self.head = nn.Sequential(
            nn.Upsample(scale_factor=(patch_size,patch_size), mode="bilinear"),
            nn.Conv2d(kwargs['ch'],1,kernel_size=1) 
        )
    ...
    def forward(self, x: torch.Tensor):
        bs,feat, emb = x.shape
        sz = np.sqrt(feat)
        x = x.permute(0,2,1).view(bs,emb,int(sz),int(sz))
        return self.head(x)

    
    
class TransCrowd(nn.Module):
    def __init__(self,img_size,patch_size=16,vit='vit_base_patch16_224',**kwargs):
        super().__init__()
        timm.models.vision_transformer.default_cfgs[vit]['input_size'] = (3,img_size,img_size)
        self.backbone = timm.create_model(vit, pretrained=True, num_classes=0)
        self.head = Head(img_size,patch_size,**kwargs)
...
net = TransCrowd(img_size=784, vit='vit_tiny_patch16_224',ch=192).cuda()
