...
#path = '/mnt/home/hheat/USERDIR/counting-bench/data/qnrf'
#path = '/home/hbaiaa/haoyue/benchmark/data/UCSD'
path = '/mnt/home/hbaiaa/USERDIR/data/FDST'


train_images = path +'/train/img'
#train_anno = path + '/train/annotations'
train_dmaps = path + '/train/ground_truth'

test_images = path +'/test/img'
#test_anno = path + '/test/annotations'
test_dmaps = path + '/test/ground_truth'
...
#         fn_2, points_2 = self._get_gt_data(idx_2)
    ...
    def _get_gt_data(self,idx):
        ...
        points = test_data['annPoints'].astype(int)
        return fn, points
...
#                     image = np.pad(image,(pad_left,pad_right),'constant')
                if h < self.crop_size:
                    ...
                    if diff % 2 ==0:
                        ...
                        pad_top = diff // 2
                        pad_bot = pad_top + 1
#                     image = np.pad(image,((0,0),(pad_top,pad_bot)),'constant')
                image = np.pad(image,((pad_top,pad_bot),(pad_left,pad_right),(0,0)),mode='constant',constant_values=0)
                density_map = np.pad(density_map,((pad_top,pad_bot),(pad_left,pad_right)),mode='constant',constant_values=0)
            h,w = image.shape[:2]
...
#             gt_points = gt_points[mask]
            density_map = cv2.resize(density_map,(self.crop_size//self.downsample,self.crop_size//self.downsample))*self.downsample*self.downsample
            
        else:
            pad_h, pad_w = self._get_pad((h,w),crop_sz=self.crop_size)
            density_map = cv2.resize(density_map,(w//self.downsample,h//self.downsample))*self.downsample*self.downsample#,interpolation=cv2.INTER_NEAREST)
            #density_map = density_map[1:-1,:]
            image = np.pad(image,((pad_h,pad_h),(pad_w,pad_w),(0,0)),'constant')
            density_map = np.pad(density_map,((pad_h,pad_h),(pad_w,pad_w)),'constant')
    ...
    def _get_pad(self, im_sz, crop_sz=896,**kwargs):
        h,w = im_sz
        h_mul = h // crop_sz + 1
        w_mul = w // crop_sz + 1
...
train_fp = glob(train_images + '/*.jpg') #jpg
test_fp = glob(test_images + '/*.jpg') #jpg
...
#             self.model = nn.DataParallel(self.model)
    ...
    def fit(self, train_loader, validation_loader):
        for e in range(self.config.n_epochs):
            ...
            summary_loss, train_mae, train_mse = self.train_one_epoch(train_loader)

            self.log(f'[RESULT]: Train. Epoch: {self.epoch}, mse_loss: {summary_loss.avg:.8f}, time: {(time.time() - t):.5f}')
            self.log(f'[RESULT]: Train. Epoch: {self.epoch}, mae: {train_mae.avg:.8f}, time: {(time.time() - t):.5f}')
            self.log(f'[RESULT]: Train. Epoch: {self.epoch}, mse: {math.sqrt(train_mse.avg):.8f}, time: {(time.time() - t):.5f}')
            ...
            if (self.epoch+1) % 20 == 0:
                ...
                self.log(f'[RESULT]: Val. Epoch: {self.epoch}, mae: {val_mae.avg:.8f}, time: {(time.time() - t):.5f}')
                self.log(f'[RESULT]: Val. Epoch: {self.epoch}, mae: {math.sqrt(val_mse.avg):.8f}, time: {(time.time() - t):.5f}')
...
#             y_range=[0,1],self_attention=True).cuda()

net = CounTR(backbone, act_cls=act_cls,
             imsize=224, layer_idx=idx,
             y_range=None, self_attention=False).cuda()
